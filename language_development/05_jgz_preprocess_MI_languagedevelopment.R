...
# this is the predictor dataset for the case taal/spraak 
predictors <- readRDS("case_jgz/repo/language_development/data/wiechen_predictors_p.rds")

#### 1.1 Prepare the predictors data set for imputation ####
predictors<-predictors%>%
  replace_with_na_at(.vars = "income_hh_source", condition = ~.x == "Huishoudensinkomen onbekend")%>%
  replace_with_na_at(.vars = "house_ownership", condition = ~.x == "Onbekend")%>%
  ...
  mutate(
    house_ownership = droplevels(house_ownership), 
    income_hh_source=droplevels(income_hh_source), 
    ...
    amww = as.numeric(levels(amww_f)[amww_f]), #too little N in some categories, converted to numeric
    AANTALPERSHHf = recode(AANTALPERSHHf, 
                    '1' = '1 of 2', # 1 person in HH is probably a mistake, merged with 2
                    '2' = '1 of 2'), 
    ...
    GBA_generation_fa=droplevels(GBA_generation_fa),
    income_hh_source = recode(income_hh_source, 
    ...
    SPOLIS_contract_mo = ifelse(is.na(SPOLIS_contract_mo) & SECM_mo=="Ontvanger werkloosheidsuitkering" | 
                      ...
                      SECM_mo=="Ontvanger uitkering ziekte/AO/pension" | 
                      SECM_mo=="Nog niet schoolg./schol./stud. met of zonder ink" | 
                      SECM_mo=="Overig zonder inkomen", "niet werkend volgens SECM", SPOLIS_contract_mo), 
    SPOLIS_contract_mo = as.factor(SPOLIS_contract_mo),
  ...
  mutate(N_vooraf_sga = factor(N_vooraf_sga, ordered = FALSE))%>%
  select(-jaar, -amww_f, -LAND_ACHTS_gebl1, -LAND_ACHTS_gebl2, -LAND_ETNG_gebl1, -LAND_ETNG_gebl2, -ZVWK_birth_obstetrician_fa, - ZVWK_birth_maternitycare_fa)%>% #birth costs father are 0, excluded
  select(-vroeg_geb_24_28 , -vroeg_geb_28_34, -vroeg_geb_34_37, -vroeg_geb_24_37) #calculate later based on imputed amww
...
taal_spraak_train <- predictors[split, ]
taal_spraak_test <- predictors[!split,]
...
predmatrix = quickpred(taal_spraak_train, mincor = 0.2)
...
#set predictors for wages and paid hours manually 
predmatrix[c("SPOLIS_paidhours_fa", "SPOLIS_paidhours_mo", "SPOLIS_wages_fa", "SPOLIS_wages_mo"), ]<-0 #first set all predictors to 0 

predmatrix["SPOLIS_paidhours_fa", c("SPOLIS_paidhours_mo", "SPOLIS_wages_fa", "SPOLIS_wages_mo", "educc_mo", "educc_fa", "ED_rentown", "SECM_mo", 
                                    "SECM_fa", "income_mo", "income_fa")]<-1
predmatrix["SPOLIS_paidhours_mo", c("SPOLIS_paidhours_fa", "SPOLIS_wages_fa", "SPOLIS_wages_mo", "educc_mo", "educc_fa", "ED_rentown", "SECM_mo", 
                                    "SECM_fa", "income_mo", "income_fa")]<-1
predmatrix["SPOLIS_wages_fa", c("SPOLIS_paidhours_fa", "SPOLIS_paidhours_mo", "SPOLIS_wages_mo", "educc_mo", "educc_fa", "ED_rentown", "SECM_mo", 
                                "SECM_fa", "income_mo", "income_fa")]<-1
predmatrix["SPOLIS_wages_mo", c("SPOLIS_paidhours_fa", "SPOLIS_paidhours_mo", "SPOLIS_wages_fa", "educc_mo", "educc_fa", "ED_rentown", "SECM_mo", 
                                "SECM_fa", "income_mo", "income_fa")]<-1

#set matualy predictors as initial imputation didn't go well (clear unwanted trends on convergence plots)
predmatrix[c("verantw_bb", "verantw_eb", "sectio_ia", "gebplaats", "robson"), ]<-0 #set all predictors to 0
...
predmatrix["robson", c("sectio_ia", "par_cat", "geboortegew", "overdracht", "pediater", "partus", "meerling", "amww")]<-1

predmatrix[c("N_vooraf_sga", "N_vroeg_24_37"),]<-0
predmatrix["N_vooraf_sga", c("lft_concept_mo", "lft_concept_fa", "par_cat", "grav_cat", "sectio_ia", "vooraf_zw_vroeg_24_37")]<-1
predmatrix["N_vroeg_24_37", c("lft_concept_mo", "lft_concept_fa", "par_cat", "grav_cat", "sectio_ia", "vooraf_sga")]<-1

predmatrix["taal_ont_2y", c("taal_ont3y9m", "zin2w", "pop6", "bst_nl", "taalomg", "educc_mo", "educc_fa")]<-1
predmatrix["bmi_2yrs_znl", c("lengte_2yrs_znl", "gewicht_2yrs_znl", "geboortegew", "educc_mo", "educc_fa")]<-1
predmatrix["lengte_2yrs_znl", c("bmi_2yrs_znl", "gewicht_2yrs_znl", "geboortegew", "educc_mo", "educc_fa")]<-1
predmatrix["gewicht_2yrs_znl", c("lengte_2yrs_znl", "bmi_2yrs_znl", "geboortegew", "educc_mo", "educc_fa")]<-1



#### 1.5 Multiple imputation with MICE ####
### Train data 
mi <- mice(taal_spraak_train, m=5, pred = predmatrix, seed = 815, maxit=100, defaultMethod = c("cart", "cart", "cart", "cart"),
           print = F) 
save(mi, file = "H:/Mirthe/case_jgz/repo/language_development/data/taal_spraak_MI.rda")
...
a<-stripplot(mi, gewicht_2yrs_znl~.imp, pch=c(21,20), cex=c(1,1.5))
...
e<-stripplot(mi, lengte_2yrs_znl~.imp, pch=c(21,20), cex=c(1,1.5))
...
i<-stripplot(mi, bmi_2yrs_znl~.imp, pch=c(21,20), cex=c(1,1.5))
...
#save mi object 
load(file="H:/Mirthe/case_jgz/repo/language_development/data/taal_spraak_MI.rda")

# MI is a mids object: get the complete data set by replacing missing values by imputations 
mi_complete <- mi %>% 
  mice::complete("all")

# ! QUESTION FOR POLINA; IF WE DON'T SET FACTORS TO ORDERED IN PREPROCESS, WE DON'T HAVE TO DO THIS
# ! I THINK RF MODELs MIGHT HAVE BEEN CREATED WITH ORDERED, BUT IT DID NOT MAKE A DIFFERENCE FOR PREDICTION 
# ! I AM NOT SURE IF IT IS ORDERED IN MI AND WHETHER THAT MATTERS FOR IMPUTATION

# POLINA: there are a few ordered factors in 'predictors' file used for me (education parents, van Wiechen items at 2j, taalomg, amddd1ond_cat etc.) These are treated as ordered in MI.
...
saveRDS(mi_complete, file="H:/Mirthe/case_jgz/repo/language_development/data/taal_spraak_MI_complete.rds")

### Test data
mi_test <- mice.mids(mi, maxit = 1, newdata = taal_spraak_test, seed = 8151)
mi_test_comp <- mi_test %>% 
  mice::complete("all")
mi_test_comp <- lapply(mi_test_comp, func_convert_ordered)
saveRDS(mi_test_comp, "case_jgz/repo/language_development/data/taal_spraak_MI_test_comp.rds")


#loading for future use
load("H:/Mirthe/case_jgz/repo/language_development/data/taal_spraak_MI.rda")
load("H:/Mirthe/case_jgz/repo/language_development/data/taal_spraak_MI_test.rda")
