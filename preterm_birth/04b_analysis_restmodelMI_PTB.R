...
mi_complete <- readRDS("case1_preterm_birth/repo/PTB_data/predictors_rest_model32_mi_complete.rds")
# look at the data set
glimpse(mi_complete)

mi_test_comp <- readRDS("case1_preterm_birth/repo/PTB_data/predictors_rest_model32_mi_test_comp.rds")
...
rf_rest <- mi_complete %>% 
  ...
  lapply(ranger, formula = premature_birth ~ ., num.trees = 1000, importance = 'impurity', probability = TRUE, seed = 8151)
...
#saveRDS(rf_rest, "case1_preterm_birth/repo/PTB_output/restricted/rf_rest.rds")
...
var_imp_norm1[1:23,] %>% # change this number in case you want to look at more independent variables in the model
...
varimp_sheet_names <- list('rf_rest_call' = rf_rest$`1`, 'var_imp_norm1' = var_imp_norm1, 'var_imp_norm2' = var_imp_norm2, 'var_imp_norm3' = var_imp_norm3)
openxlsx::write.xlsx(varimp_sheet_names, file = 'case1_preterm_birth/repo/PTB_output/restricted/RF_VarImp_rest32_MI.xlsx')
...
# try different thresholds; threshold at prevalence = 0.81, threshold for 8% highest risks, threshold for 15% highest risks
# you can do this for all m = 3 rf_rest models by changing rf_rest$`1` to rf_rest$`2` or rf_rest$`3`
rf_rest_prob <- ifelse(rf_rest$`1`$predictions[,2] > 0.056, 1, 0) %>% as.factor()  
...
rf_rest_cm <- confusionMatrix(rf_rest_prob, mi_complete$`1`$premature_birth, positive = "1")
rf_rest_cm
rf_rest_cmp <- round(prop.table(confusionMatrix(rf_rest_prob, mi_complete$`1`$premature_birth, positive = "1")$table),4)*100
rf_rest_cmp
...
write_excel_csv2(pred_rf_rest_pool, "case1_preterm_birth/repo/PTB_output/restricted/pred_rf_rest32_pool.csv")
...
pred_rf_rest_pool_class1 <- ifelse(pred_rf_rest_pool$pred_outcome_prob > 0.056, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_pool_class_cm1 <- confusionMatrix(pred_rf_rest_pool_class1, mi_test_comp$'1'$premature_birth, positive = "1")
pred_rf_rest_pool_class_cm1
pred_rf_rest_pool_class_cmp1 <- round(prop.table(confusionMatrix(pred_rf_rest_pool_class1, mi_test_comp$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_pool_class_cmp1

# try different thresholds: > 0.0195 approx. 10% highest risks
pred_rf_rest_pool_class10 <- ifelse(pred_rf_rest_pool$pred_outcome_prob > 0.0195, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_pool_class_cm10 <- confusionMatrix(pred_rf_rest_pool_class10, mi_test_comp$'1'$premature_birth, positive = "1")
pred_rf_rest_pool_class_cm10
pred_rf_rest_pool_class_cmp10 <- round(prop.table(confusionMatrix(pred_rf_rest_pool_class10, mi_test_comp$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_pool_class_cmp10

# try different thresholds: fixed false positive rate 10%, > 0.0191
pred_rf_rest_pool_class10 <- ifelse(pred_rf_rest_pool$pred_outcome_prob > 0.0191, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_pool_class_cm10 <- confusionMatrix(pred_rf_rest_pool_class10, mi_test_comp$'1'$premature_birth, positive = "1")
pred_rf_rest_pool_class_cm10
pred_rf_rest_pool_class_cmp10 <- round(prop.table(confusionMatrix(pred_rf_rest_pool_class10, mi_test_comp$'1'$premature_birth, positive = "1")$table),4)*100
...
# try different thresholds: >= 0.0138 approx. 15% highest risks
pred_rf_rest_pool_class15 <- ifelse(pred_rf_rest_pool$pred_outcome_prob > 0.01357, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_pool_class_cm15 <- confusionMatrix(pred_rf_rest_pool_class15, mi_test_comp$'1'$premature_birth, positive = "1")
pred_rf_rest_pool_class_cm15
pred_rf_rest_pool_class_cmp15 <- round(prop.table(confusionMatrix(pred_rf_rest_pool_class15, mi_test_comp$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_pool_class_cmp15

# try different thresholds: > 0.01046 approx. 20% highest risks
pred_rf_rest_pool_class20 <- ifelse(pred_rf_rest_pool$pred_outcome_prob > 0.01046, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_pool_class_cm20 <- confusionMatrix(pred_rf_rest_pool_class20, mi_test_comp$'1'$premature_birth, positive = "1")
pred_rf_rest_pool_class_cm20
pred_rf_rest_pool_class_cmp20 <- round(prop.table(confusionMatrix(pred_rf_rest_pool_class20, mi_test_comp$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_pool_class_cmp20

# look at the area under the ROC curve to assess overall model performance
rf_rest_roc <- roc(mi_test_comp$'1'$premature_birth, pred_rf_rest_pool$pred_outcome_prob)
auc(rf_rest_roc)
plot(rf_rest_roc, print.auc = TRUE, col = "black", xlim = c(1, 0), ylim=c(0, 1))
saveRDS(rf_rest_roc, "case1_preterm_birth/repo/PTB_output/restricted/rf_rest32_roc.rds") 
...
pred_rf_rest_pool_classb <- ifelse(pred_rf_rest_pool$pred_outcome_prob > 0.008197139, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_pool_class_cmb <- confusionMatrix(pred_rf_rest_pool_classb, mi_test_comp$'1'$premature_birth, positive = "1")
pred_rf_rest_pool_class_cmb
pred_rf_rest_pool_class_cmpb <- round(prop.table(confusionMatrix(pred_rf_rest_pool_classb, mi_test_comp$'1'$premature_birth, positive = "1")$table),4)*100
...
pred_rf_rest_pool$premature_birth <- mi_test_comp$'1'$premature_birth
table(pred_rf_rest_pool$premature_birth)
prop.table(table(pred_rf_rest_pool$premature_birth)) * 100

# look at predicted probabilities by PTB outcome
PTB_rf_rest_pool_cdf_1 <- pred_rf_rest_pool %>% filter(premature_birth == 1)
PTB_rf_rest_pool_cdf_0 <- pred_rf_rest_pool %>% filter(premature_birth == 0)
...
write_excel_csv2(PTB_rf_rest_pool_cdf_1_list, "case1_preterm_birth/repo/PTB_output/restricted/PTB32_rf_rest_pool_cdf_1_list.csv")
write_excel_csv2(PTB_rf_rest_pool_cdf_0_list, "case1_preterm_birth/repo/PTB_output/restricted/PTB32_rf_rest_pool_cdf_0_list.csv")
...
prop.table(table(mi_test_comp_multi$'1'$premature_birth))*100
...
# save the predicted probabilities for future use
write_excel_csv2(pred_rf_rest_multi_pool, "case1_preterm_birth/repo/PTB_output/restricted/pred_rf_rest_multi32_pool.csv")

# classify predictions of test data; predict binary outcome using a predicted probability threshold
# this threshold is set a prevalence, we predict a high risk of PTB for approx. 0.556% of pregnancies
pred_rf_rest_multi_pool_class1 <- ifelse(pred_rf_rest_multi_pool$pred_outcome_prob > 0.09, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_multi_pool_class_cm1 <- confusionMatrix(pred_rf_rest_multi_pool_class1, mi_test_comp_multi$'1'$premature_birth, positive = "1")
pred_rf_rest_multi_pool_class_cm1
pred_rf_rest_multi_pool_class_cmp1 <- round(prop.table(confusionMatrix(pred_rf_rest_multi_pool_class1, mi_test_comp_multi$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_multi_pool_class_cmp1

# try different thresholds: > 0.013 approx. 10% highest risks
pred_rf_rest_multi_pool_class10 <- ifelse(pred_rf_rest_multi_pool$pred_outcome_prob > 0.0128, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_multi_pool_class_cm10 <- confusionMatrix(pred_rf_rest_multi_pool_class10, mi_test_comp_multi$'1'$premature_birth, positive = "1")
pred_rf_rest_multi_pool_class_cm10
pred_rf_rest_multi_pool_class_cmp10 <- round(prop.table(confusionMatrix(pred_rf_rest_multi_pool_class10, mi_test_comp_multi$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_multi_pool_class_cmp10

# try different thresholds: fixed false positive rate 10%, > 0.0125
pred_rf_rest_multi_pool_class10 <- ifelse(pred_rf_rest_multi_pool$pred_outcome_prob > 0.0125, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_multi_pool_class_cm10 <- confusionMatrix(pred_rf_rest_multi_pool_class10, mi_test_comp_multi$'1'$premature_birth, positive = "1")
pred_rf_rest_multi_pool_class_cm10
pred_rf_rest_multi_pool_class_cmp10 <- round(prop.table(confusionMatrix(pred_rf_rest_multi_pool_class10, mi_test_comp_multi$'1'$premature_birth, positive = "1")$table),4)*100
...
# try different thresholds: > 0.00882 approx. 15% highest risks
pred_rf_rest_multi_pool_class15 <- ifelse(pred_rf_rest_multi_pool$pred_outcome_prob > 0.00882, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_multi_pool_class_cm15 <- confusionMatrix(pred_rf_rest_multi_pool_class15, mi_test_comp_multi$'1'$premature_birth, positive = "1")
pred_rf_rest_multi_pool_class_cm15
pred_rf_rest_multi_pool_class_cmp15 <- round(prop.table(confusionMatrix(pred_rf_rest_multi_pool_class15, mi_test_comp_multi$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_multi_pool_class_cmp15

# try different thresholds: > 0.01046 approx. 20% highest risks
pred_rf_rest_multi_pool_class20 <- ifelse(pred_rf_rest_multi_pool$pred_outcome_prob > 0.0067, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_multi_pool_class_cm20 <- confusionMatrix(pred_rf_rest_multi_pool_class20, mi_test_comp_multi$'1'$premature_birth, positive = "1")
pred_rf_rest_multi_pool_class_cm20
pred_rf_rest_multi_pool_class_cmp20 <- round(prop.table(confusionMatrix(pred_rf_rest_multi_pool_class20, mi_test_comp_multi$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_multi_pool_class_cmp20

# look at the area under the ROC curve to assess overall model performance
rf_rest_multi_roc <- roc(mi_test_comp_multi$'1'$premature_birth, pred_rf_rest_multi_pool$pred_outcome_prob)
auc(rf_rest_multi_roc)
plot(rf_rest_multi_roc, print.auc = TRUE, col = "black", xlim = c(1, 0), ylim=c(0, 1))
saveRDS(rf_rest_multi_roc, "case1_preterm_birth/repo/PTB_output/restricted/rf_rest32_multi_roc.rds") 
...
pred_rf_rest_multi_pool_classb <- ifelse(pred_rf_rest_multi_pool$pred_outcome_prob > 0.00455598, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_multi_pool_class_cmb <- confusionMatrix(pred_rf_rest_multi_pool_classb, mi_test_comp_multi$'1'$premature_birth, positive = "1")
pred_rf_rest_multi_pool_class_cmb
pred_rf_rest_multi_pool_class_cmpb <- round(prop.table(confusionMatrix(pred_rf_rest_multi_pool_classb, mi_test_comp_multi$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_multi_pool_class_cmpb

### save the predicted probabilities by outcome: sample of multipara
# add the observed outcome to the data frame with predicted probabilities 
pred_rf_rest_multi_pool$premature_birth <- mi_test_comp_multi$'1'$premature_birth
table(pred_rf_rest_multi_pool$premature_birth)
prop.table(table(pred_rf_rest_multi_pool$premature_birth)) * 100

# look at predicted probabilities by PTB outcome
PTB_rf_rest_multi_pool_cdf_1 <- pred_rf_rest_multi_pool %>% filter(premature_birth == 1)
PTB_rf_rest_multi_pool_cdf_0 <- pred_rf_rest_multi_pool %>% filter(premature_birth == 0)
...
write_excel_csv2(PTB_rf_rest_multi_pool_cdf_1_list, "case1_preterm_birth/repo/PTB_output/restricted/PTB32_rf_rest_multi_pool_cdf_1_list.csv")
write_excel_csv2(PTB_rf_rest_multi_pool_cdf_0_list, "case1_preterm_birth/repo/PTB_output/restricted/PTB32_rf_rest_multi_pool_cdf_0_list.csv")
# with these saved, you can later assess classification performance at various thresholds of predicted probability
...
mi_test_comp_primi <- lapply(mi_test_comp, function(mi_list){
  filter(mi_list, interpreg_cat == "nvt" & N_vroeg_24_37 == "nvt")})
table(mi_test_comp_primi$'2'$premature_birth)
prop.table(table(mi_test_comp_primi$'1'$premature_birth))*100
...
# save the predicted probabilities for future use
write_excel_csv2(pred_rf_rest_primi_pool, "case1_preterm_birth/repo/PTB_output/restricted/pred_rf_rest_primi32_pool.csv")

# classify predictions of test data; predict binary outcome using a predicted probability threshold
# this threshold is set a prevalence, we predict a high risk of PTB for approx. 1.097% of pregnancies
pred_rf_rest_primi_pool_class1 <- ifelse(pred_rf_rest_primi_pool$pred_outcome_prob > 0.0461, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_primi_pool_class_cm1 <- confusionMatrix(pred_rf_rest_primi_pool_class1, mi_test_comp_primi$'1'$premature_birth, positive = "1")
pred_rf_rest_primi_pool_class_cm1
pred_rf_rest_primi_pool_class_cmp1 <- round(prop.table(confusionMatrix(pred_rf_rest_primi_pool_class1, mi_test_comp_primi$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_primi_pool_class_cmp1

# try different thresholds: > 0.0237 approx. 10% highest risks
pred_rf_rest_primi_pool_class10 <- ifelse(pred_rf_rest_primi_pool$pred_outcome_prob > 0.0237, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_primi_pool_class_cm10 <- confusionMatrix(pred_rf_rest_primi_pool_class10, mi_test_comp_primi$'1'$premature_birth, positive = "1")
pred_rf_rest_primi_pool_class_cm10
pred_rf_rest_primi_pool_class_cmp10 <- round(prop.table(confusionMatrix(pred_rf_rest_primi_pool_class10, mi_test_comp_primi$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_primi_pool_class_cmp10

# try different thresholds: fixed false positive rate 10%, > 0.0234
pred_rf_rest_primi_pool_class10 <- ifelse(pred_rf_rest_primi_pool$pred_outcome_prob > 0.0234, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_primi_pool_class_cm10 <- confusionMatrix(pred_rf_rest_primi_pool_class10, mi_test_comp_primi$'1'$premature_birth, positive = "1")
pred_rf_rest_primi_pool_class_cm10
pred_rf_rest_primi_pool_class_cmp10 <- round(prop.table(confusionMatrix(pred_rf_rest_primi_pool_class10, mi_test_comp_primi$'1'$premature_birth, positive = "1")$table),4)*100
...
# try different thresholds: > 0.019 approx. 15% highest risks
pred_rf_rest_primi_pool_class15 <- ifelse(pred_rf_rest_primi_pool$pred_outcome_prob > 0.019, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_primi_pool_class_cm15 <- confusionMatrix(pred_rf_rest_primi_pool_class15, mi_test_comp_primi$'1'$premature_birth, positive = "1")
pred_rf_rest_primi_pool_class_cm15
pred_rf_rest_primi_pool_class_cmp15 <- round(prop.table(confusionMatrix(pred_rf_rest_primi_pool_class15, mi_test_comp_primi$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_primi_pool_class_cmp15

# try different thresholds: > 0.0145 approx. 20% highest risks
pred_rf_rest_primi_pool_class20 <- ifelse(pred_rf_rest_primi_pool$pred_outcome_prob > 0.0145, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_primi_pool_class_cm20 <- confusionMatrix(pred_rf_rest_primi_pool_class20, mi_test_comp_primi$'1'$premature_birth, positive = "1")
pred_rf_rest_primi_pool_class_cm20
pred_rf_rest_primi_pool_class_cmp20 <- round(prop.table(confusionMatrix(pred_rf_rest_primi_pool_class20, mi_test_comp_primi$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_primi_pool_class_cmp20

# look at the area under the ROC curve to assess overall model performance
rf_rest_primi_roc <- roc(mi_test_comp_primi$'1'$premature_birth, pred_rf_rest_primi_pool$pred_outcome_prob)
auc(rf_rest_primi_roc)
plot(rf_rest_primi_roc, print.auc = TRUE, col = "black", xlim = c(1, 0), ylim=c(0, 1))
saveRDS(rf_rest_primi_roc, "case1_preterm_birth/repo/PTB_output/restricted/rf_rest32_primi_roc.rds") 
...
pred_rf_rest_primi_pool_classb <- ifelse(pred_rf_rest_primi_pool$pred_outcome_prob > 0.01427921, 1, 0) %>% as.factor() 
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_rf_rest_primi_pool_class_cmb <- confusionMatrix(pred_rf_rest_primi_pool_classb, mi_test_comp_primi$'1'$premature_birth, positive = "1")
pred_rf_rest_primi_pool_class_cmb
pred_rf_rest_primi_pool_class_cmpb <- round(prop.table(confusionMatrix(pred_rf_rest_primi_pool_classb, mi_test_comp_primi$'1'$premature_birth, positive = "1")$table),4)*100
pred_rf_rest_primi_pool_class_cmpb

### save the predicted probabilities by outcome: sample of primipara
# add the observed outcome to the data frame with predicted probabilities 
pred_rf_rest_primi_pool$premature_birth <- mi_test_comp_primi$'1'$premature_birth
table(pred_rf_rest_primi_pool$premature_birth)
prop.table(table(pred_rf_rest_primi_pool$premature_birth)) * 100

# look at predicted probabilities by PTB outcome
PTB_rf_rest_primi_pool_cdf_1 <- pred_rf_rest_primi_pool %>% filter(premature_birth == 1)
PTB_rf_rest_primi_pool_cdf_0 <- pred_rf_rest_primi_pool %>% filter(premature_birth == 0)
...
write_excel_csv2(PTB_rf_rest_primi_pool_cdf_1_list, "case1_preterm_birth/repo/PTB_output/restricted/PTB32_rf_rest_primi_pool_cdf_1_list.csv")
write_excel_csv2(PTB_rf_rest_primi_pool_cdf_0_list, "case1_preterm_birth/repo/PTB_output/restricted/PTB32_rf_rest_primi_pool_cdf_0_list.csv")
# with these saved, you can later assess classification performance at various thresholds of predicted probability
...
glm_rest <- mi_complete %>% 
  lapply(glm, formula = premature_birth ~ ., family = binomial)
...
#saveRDS(glm_rest, "case1_preterm_birth/repo/PTB_output/restricted/glm_rest32.rds")
# maybe save the pooled summary instead
write_excel_csv2(glm_rest_pool, "case1_preterm_birth/repo/PTB_output/restricted/glm_rest32_pool.csv")
...
# save the predicted probabilities for future use
write_excel_csv2(pred_glm_rest_pool, "case1_preterm_birth/repo/PTB_output/restricted/pred_glm_rest32_pool.csv")
...
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_glm_rest_pool_class_cm <- confusionMatrix(pred_glm_rest_pool_class, mi_test_comp$'1'$premature_birth, positive = "1")
pred_glm_rest_pool_class_cm
pred_glm_rest_pool_class_cmp <- round(prop.table(confusionMatrix(pred_glm_rest_pool_class, mi_test_comp$'1'$premature_birth, positive = "1")$table),4)*100
...
# look at the area under the ROC curve to assess overall model performance
glm_rest_roc <- roc(mi_test_comp$'1'$premature_birth, pred_glm_rest_pool$pred_outcome_prob_exp)
auc(glm_rest_roc)
plot(glm_rest_roc, print.auc = TRUE, col = "black", xlim = c(1, 0), ylim=c(0, 1))
saveRDS(glm_rest_roc, "case1_preterm_birth/repo/PTB_output/restricted/glm_rest_roc.rds") 


### save the predicted probabilities by outcome; population sample
# add the observed outcome to the data frame with predicted probabilities 
pred_glm_rest_pool$premature_birth <- mi_test_comp$'1'$premature_birth
table(pred_glm_rest_pool$premature_birth)
prop.table(table(pred_glm_rest_pool$premature_birth)) * 100

# look at predicted probabilities by PTB outcome
PTB_glm_rest_pool_cdf_1 <- pred_glm_rest_pool %>% filter(premature_birth == 1)
PTB_glm_rest_pool_cdf_0 <- pred_glm_rest_pool %>% filter(premature_birth == 0)
...
write_excel_csv2(PTB_glm_rest_pool_cdf_1_list, "case1_preterm_birth/repo/PTB_output/restricted/PTB_glm_rest32_pool_cdf_1_list.csv")
write_excel_csv2(PTB_glm_rest_pool_cdf_0_list, "case1_preterm_birth/repo/PTB_output/restricted/PTB_glm_rest32_pool_cdf_0_list.csv")
# with these saved, you can later assess classification performance at various thresholds of predicted probability
...
# you can save the trained model but this is a very big object
#saveRDS(glm_rest_adapted, "case1_preterm_birth/repo/PTB_output/restricted/glm_rest_adapted.rds")
# maybe save the pooled summary instead
write_excel_csv2(glm_rest_adapted_pool, "case1_preterm_birth/repo/PTB_output/restricted/glm_rest_adapted_pool.csv")
...
# save the predicted probabilities for future use
write_excel_csv2(pred_glm_rest_adapted_pool, "case1_preterm_birth/repo/PTB_output/restricted/pred_glm_rest_adapted_pool.csv")
...
# look at the confusionMatrix: accuracy, sensitivity, specificity and the Cohen's Kappa coefficient (Kappa accounts for the possibility of a correct prediction by chance alone (in contrast to Accuracy))
pred_glm_rest_adapted_pool_class_cm <- confusionMatrix(pred_glm_rest_adapted_pool_class, mi_test_comp_adapted$'1'$premature_birth, positive = "1")
pred_glm_rest_adapted_pool_class_cm
pred_glm_rest_adapted_pool_class_cmp <- round(prop.table(confusionMatrix(pred_glm_rest_adapted_pool_class, mi_test_comp_adapted$'1'$premature_birth, positive = "1")$table),4)*100
...
# look at the area under the ROC curve to assess overall model performance
glm_rest_adapted_roc <- roc(mi_test_comp_adapted$'1'$premature_birth, pred_glm_rest_adapted_pool$pred_outcome_prob_exp)
auc(glm_rest_adapted_roc)
plot(glm_rest_adapted_roc, print.auc = TRUE, col = "black", xlim = c(1, 0), ylim=c(0, 1))
saveRDS(glm_rest_adapted_roc, "case1_preterm_birth/repo/PTB_output/restricted/glm_rest_adapted_roc.rds") 


### save the predicted probabilities by outcome; population sample
# add the observed outcome to the data frame with predicted probabilities 
pred_glm_rest_adapted_pool$premature_birth <- mi_test_comp_adapted$'1'$premature_birth
table(pred_glm_rest_adapted_pool$premature_birth)
prop.table(table(pred_glm_rest_adapted_pool$premature_birth)) * 100

# look at predicted probabilities by PTB outcome
PTB_glm_rest_adapted_pool_cdf_1 <- pred_glm_rest_adapted_pool %>% filter(premature_birth == 1)
PTB_glm_rest_adapted_pool_cdf_0 <- pred_glm_rest_adapted_pool %>% filter(premature_birth == 0)
...
write_excel_csv2(PTB_glm_rest_adapted_pool_cdf_1_list, "case1_preterm_birth/repo/PTB_output/restricted/PTB_glm_rest_adapted_pool_cdf_1_list.csv")
write_excel_csv2(PTB_glm_rest_adapted_pool_cdf_0_list, "case1_preterm_birth/repo/PTB_output/restricted/PTB_glm_rest_adapted_pool_cdf_0_list.csv")
# with these saved, you can later assess classification performance at various thresholds of predicted probability
...
# full models
rf_full_roc<- readRDS("case1_preterm_birth/repo/PTB_output/restricted/rf_full32_roc.rds")
auc(rf_full_roc)

rf_rest_roc <- readRDS("case1_preterm_birth/repo/PTB_output/restricted/rf_rest32_roc.rds")
auc(rf_rest_roc)
rf_rest_multi_roc <- readRDS("case1_preterm_birth/repo/PTB_output/restricted/rf_rest32_multi_roc.rds")
auc(rf_rest_multi_roc)
rf_rest_primi_roc <- readRDS("case1_preterm_birth/repo/PTB_output/restricted/rf_rest32_primi_roc.rds")
auc(rf_rest_primi_roc)
