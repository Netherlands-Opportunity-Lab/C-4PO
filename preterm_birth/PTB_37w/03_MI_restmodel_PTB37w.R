...
predictors <- readRDS("case1_preterm_birth/PTB_data/ptb_37weeks/predictors_rest_model.rds")
glimpse(predictors)

# If multiple imputations have been generated before, you can simply load the mids object, skip to line 150

# QUESTION TO POLINA: is the MI script for PTB <32w the same as for PTB <37w? ####
...
predmatrix[,c("year_conception", "interpreg_nvt","age_at_concp_fa_missing", "Rinpersoons_Kind", "Rinpersoon_Kind", "Rinpersoons_Moeder", "Rinpersoon_Moeder", "rins_fa", "rin_fa")]<- 0  # leave these vars from the set of predictors
predmatrix[c("year_conception", "interpreg_nvt","age_at_concp_fa_missing", "Rinpersoons_Kind", "Rinpersoon_Kind", "Rinpersoons_Moeder", "Rinpersoon_Moeder", "rins_fa", "rin_fa"), ]<- 0  #leave these vars from vars to be predicted


# use mice to generate multiple imputations
# be aware: this takes a long time to run
start_time<-Sys.time()
mi <- mice(vroeg_geb_train, m=3, pred = predmatrix, seed = 815, maxit=40, defaultMethod = c("cart", "cart", "cart", "cart"),
           print = F) 
end_time<-Sys.time()
...
save(mi, file="H:/Mirthe/case1_preterm_birth/PTB_data/MI_vroeggeb_with_year.rda")
...
save(mi_test, file="H:/Mirthe/case1_preterm_birth/PTB_data/MItest_vroeggeb_with_year_test.rda")


# !!! CHECK: MAKE SURE THE OUTCOME VARIABLE IS IN THE MI SET ####
...
load("H:/Mirthe/case1_preterm_birth/PTB_data/MI_vroeggeb_with_year.rda")
load("H:/Mirthe/case1_preterm_birth/PTB_data/MItest_vroeggeb_with_year_test.rda")
...
table(mi_complete$'1'$premature_birth37)
prop.table(table(mi_complete$'1'$premature_birth37)) * 100
...
func_postprocess_cat <- function(mi_list) {
  mi_list %>% 
    ...
    select(-c(age_at_concp_fa, age_at_concp_fa_missing, interpreg, interpreg_nvt, amddd1ond, grav))
}
...
table(mi_complete$'1'$grav_cat)
class(mi_complete$'1'$grav_cat)
...
func_postprocess_N_vroeg <- function(mi_list) {
  mi_list %>% 
    mutate(
      N_vroeg_24_28 = if_else(is.na(N_vroeg_24_28), "0", N_vroeg_24_28), 
      ...
      N_vroeg_24_37 = fct_collapse(N_vroeg_24_37, "3+" = c("3+", "4", "4+", "5+")),
      N_vroeg_24_28 = as.factor(N_vroeg_24_28))
}
...
# !!! delect preterm_birth if the outcome is still in the dataset 

# save the transformed, multiple imputed predictors data
saveRDS(mi_complete, "case1_preterm_birth/PTB_data/ptb_37weeks/predictors_rest_model37_mi_complete.rds")
...
# save the transformed, multiple imputed predictors data
saveRDS(mi_test_comp, "case1_preterm_birth/PTB_data/ptb_37weeks/predictors_rest_model37_mi_test_comp.rds")
