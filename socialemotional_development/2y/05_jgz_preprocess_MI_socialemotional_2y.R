...
predictors <- readRDS("case_jgz/data_outcome/sdq_2y/sdq_predictors_p.rds")
...
predictors<-predictors%>%
  ...
  select(-meertaal, -bst_nl) # these 2 variables are >99% missings, I think it makes little sense to impute? 


#### 1.2 Explore missingness patterns ####
miss_var_smr<-miss_var_summary(predictors)
miss_case_smr<-miss_case_summary(predictors) # quite a lot of cases with almost all missings
...
sdq_2jaar_train <- predictors[split, ]
sdq_2jaar_test <- predictors[!split, ]
...
predmatrix = quickpred(sdq_2jaar_train, mincor = 0.2)
...
predmatrix[c("geboortegew", "amww"), "robson"]<-0
...
predmatrix["SPOLIS_paidhours_fa", c("SPOLIS_wages_fa", "educc_fa", "ED_rentown","SECM_fa", "income_fa")]<-1
predmatrix["SPOLIS_paidhours_mo", c("SPOLIS_wages_mo", "educc_mo", "ED_rentown", "SECM_mo", "income_mo")]<-1
predmatrix["SPOLIS_wages_fa", c("SPOLIS_paidhours_fa", "educc_fa", "ED_rentown", "SECM_fa", "income_fa")]<-1
predmatrix["SPOLIS_wages_mo", c("SPOLIS_paidhours_mo", "educc_mo", "ED_rentown", "SECM_mo", "income_mo")]<-1
...
predmatrix[c("bmi_2yrs_znl", "lengte_2yrs_znl", "gewicht_2yrs_znl"),]<-0
...
predmatrix[c("SECM_employee_fa", "SECM_selfemployed_fa"),]<-0
predmatrix["SECM_employee_fa", c("ED_rentown", "SECM_selfemployed_fa", "income_hh_source", "SECM_fa")]<-1
predmatrix["SECM_selfemployed_fa", c("ED_rentown", "SECM_employee_fa", "income_hh_source", "SECM_fa")]<-1


#### 1.5 Multiple imputation with MICE ####
### Train data
mi <- mice(sdq_2jaar_train, m=5, pred = predmatrix, seed = 815, maxit=150, defaultMethod = c("cart", "cart", "cart", "cart"),
           print = F) 
...
a<-stripplot(mi, lengte_2yrs_znl~.imp, pch=c(21,20), cex=c(1,1.5))
...
e<-stripplot(mi, gewicht_2yrs_znl     ~.imp, pch=c(21,20), cex=c(1,1.5))
...
# save mi object 
save(mi, file="H:/Mirthe/case_jgz/MI_datasets_and_mice_scripts/sdq_2jaar_MI.rda")
# load for future use
#load(file="H:/Mirthe/case_jgz/MI_datasets_and_mice_scripts/sdq_2jaar_MI.rda")
...
saveRDS(mi_complete, "case_jgz/repo/socialemotional_development/2y/data/socialemotional_2y_MI_complete.rda")

### Test data
# check categories- of N_vooraf_sga, there are two little observations in category 3, so we collapse
table(sdq_2jaar_test$N_vooraf_sga)
sdq_2jaar_test$N_vooraf_sga <- fct_collapse(sdq_2jaar_test$N_vooraf_sga, "1+" = c("1+", "3"))

mi_test <- mice.mids(mi, maxit = 1, newdata = sdq_2jaar_test, seed = 8151)
...
saveRDS(mi_test_comp, "case_jgz/repo/socialemotional_development/2y/data/socialemotional_2y_MI_test_comp.rda")

# now that we have used MICE for multiple imputation to deal with the missing data
# we can move to the analysis: 06_jgz_prediction_socialemotional_2y.R
